...
function rectangle()
{
    ...
    for (ii = 0.; ii <= xRes; ii+=1) {
        for (jj = 0; jj <= yRes; jj+=1) {
            i = ii *2 * Math.PI / (xRes);
            j = (jj - (yRes/2.0))/(yRes/2.0);
            points.push((R+j/2.0*Math.cos(i/2.0))*Math.cos(i),(R+j/2.0*Math.cos(i/2.0))*Math.sin(i),j/2.0*Math.sin(i/2));
            UVs.push(i / (2*Math.PI), jj/40);
            var vecUse1 = vec3(R* Math.cos(i)*Math.sin(i/2.0), R * Math.sin(i)*Math.sin(i/2.0), R * (-Math.cos(i/2.0)));
            var vecUse2 = vec3(j/4.0*(Math.sin(i)*Math.cos(i)-Math.sin(i)),j/4.0*(Math.sin(i)*Math.sin(i)+Math.cos(i)),j/4.0*(-Math.cos(i)-1));
            var vecUse3;
            vecUse3 = add(vecUse1, vecUse2);
            
            normals.push(normalize(vecUse3));
...
function initTexture() {
    ...
    squareImage.src = "Material/worldmap.png";
}
